...
Although event detection has been a research topic for a long while\cite{allan1998topic}, but event detection in microblog stream is still challenging\cite{atefeh2015survey}.
According to \cite{huang2016probabilistic}, the characteristics of microblog is fast changing, high noise and short length.
...
For this reason, knowledge base can be a good supplementary for event detection on microblog stream.
For example, the tweet ``''

But it's non-trivial to perform transfer learning from knowledge base to microblog stream.
...
Due to the limit of NPMI computing module\footnote{\url{https://github.com/AKSW/Palmetto}}, which computes the coherence of up to 10 words each time, we divide the top 100 words into 20 groups. 
...
\begin{figure}
        ...
        \caption{The comparison on NPMI between LDA's topics and history states initialized from Wikipedia.}
\end{figure}
%\end{comment}
