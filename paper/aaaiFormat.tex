...
\begin{figure}[h]
    ...
    \caption{Illustration on how to initialize history states from Wikipedia (an example on the initialization of the normal states about \textit{Military}).}
...
Firstly, we demonstrate the effectiveness of history states. 
...
Due to the developer policy of Twitter\footnote{\url{https://dev.twitter.com/overview/terms/policy}}, \cite{petrovic2012using} only redistributes tweets' IDs\footnote{\url{http://demeter.inf.ed.ac.uk/cross/docs/fsd_corpus.tar.gz}}.
...
We do continous learning on the 
...
\begin{table*}[ht]
\centering
\caption{Normal States initialized from Wikipedia and topical words learned from HS-Prior-LDA}
\label{my-label}
\scalebox{0.76}{
...
To enlarge the realistic ground-truth .
\textbf{TODO.}
The dataset does not come with ground truth labels on all realistic events within the data collection period. 
Because it is infeasible to manually label the over 4 million tweetsin the dataset, we choose to manually evaluate the detected events
returned by Twevent. 
We use precision and recall to evaluate the
accuracy of the events detected.
%为了扩大我们的标注数据集，我们
\begin{table}[h]
\scriptsize
\centering
