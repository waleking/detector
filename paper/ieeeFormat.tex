...
%The experiments verifed that the 
...
The set of events' candidate words \(\mathcal{B}_{c,t}\) are defined by the words that belong to the category \(c\) in time \(t\), but are different from the normal states \(\bm{f}_c\). 
\end{rmk}
...
\label{tbl:notations}
\begin{tabular}{|c|p{0.06\columnwidth}|p{0.64\columnwidth}|} \hline
%first part: normalization initialization
\multirow{5}{0.15\columnwidth}{Normal States Initialization} & \(G^{(0)}\) & Taxonomy's Graph in Knowledge Base\\ \cline{2-3}
...
\multirow{4}{0.15\columnwidth}{Normal States Maintenance} & \(\lambda\) & normal state weight in  NS-Prior-LDA\\ \cline{2-3}
...
\multirow{5}{0.15\columnwidth}{Event Detection} & \(\mathcal{B}_{c, t}\)  & event words set detected from topic \(c\) at time \(t\)  \\ \cline{2-3}
...
\item Taxonomy Graph \(G^{(0)}\). The directed edges \(G^{(0)}\) represent the class-subclass relations in the knowledge base. 
...
This graph is constructed by Wikipedia users, and still contain
PageRank-HITS\cite{Yan:2015wq}. 
...
soc.religion.christian   & Category:Christians             \\ \Xhline{3\arrayrulewidth}
LDA & 0.74\\ \hline
BOW & 0.84\\ \hline
doc2vec & 0.73\\ \hline
