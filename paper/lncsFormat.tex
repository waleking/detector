...
\textbf{Event Detection.} According to how much data are used, the approaches are divided into two groups: the detection only without extra information, and the detection by leveraging extra information. 

Most existing methods belong to the first group.
They utilize the bursty pattern in the text stream to detect events, and are implemented by  clustering articles\cite{Allan:2000wu,Petrovic:2010uj,Wurzer:2015wq}, analyzing word frequencies\cite{Mathioudakis:2010fc,Weng:2011wz}, or finding bursty topics\cite{Diao:2012wj,Yan:2015wm}. 
...
(2) By analyzing word frequencies, 
(3) By finding bursty topics, TimeUserLDA, BurstyBTM needs the appropriate number of topics to run the topic modelling, and detects the ``large" events that associate with many articles but ignores the ``small" ones. 
