...
We develop a novel lightweight transfer learning method \textsc{LTDetector} to deal with the task.  
\textsc{LTDetector} bases on two facts, that microblog is short but can be enriched by knowledge base semantically with transfer learning; and to make the transfer efficiently and effectively, the scale of knowledge should shrink without degrading. 
The following contributions are made in \textsc{LTDetector}.
(1) We formalize the knowledge base's hierarchical structure into three parts including taxonomy graph, category-page bipartite graph, and page-content map; and further shrink the scale of knowledge to category-word level by utilizing the structure.
Specifically, category's highly related words and their chi-square scores, namely category's \textit{KB State}, are condensed.
%The knowledge base's hierarchical structure is formalized as three-part: taxonomy graph, category-page bipartite graph, and page-content map.
%The structure is utilized to condense category's highly related words and their chi-square scores, namely category's \textit{KB State}, shrinking the scale of knowledge to category-word level.
(2) Category's related words in microblog stream, namely category's \textit{Stream State}, are learned from our proposed KB-Prior-LDA, which exploits the word co-occurrences and transfers the condensed knowledge \textit{KB State} into microblogs. 
(3) Events are further detected on meaningful categories' \textit{Stream States}, which filter out the noise accurately. 
(4) Experiments on the benchmark \textit{Edinburgh twitter corpus (30 million tweets)} validate the effectiveness of our proposed transfer learning method.
\textsc{LTDetector} achieves high accuracy, promoting the precision by 9\% without  sacrificing the recall rate.
...
In our paper, to balance the performance and the cost of transfer learning, new data structure \textit{history states} is proposed. 
...
More than that, we propose a new quick and lightweight probabilistic model \textsc{HS-Prior-LDA} for transfer learning. 
After deriving \textsc{HS-Prior-LDA} (detailed in Section \ref{subsec:rs_initialization}), it reveals that whether a word in microblog links to a high-level category in knowledge base depends on three factors, the word's score in the category's \textit{history state}, the linkages of microblogs' other words, the linkages of the word types in the same time window. 
All these statistics are easy to gain, which leads the model efficient. 
What \textsc{HS-Prior-LDA} learned is summarized as \textit{recent states}, shown in Figure \ref{fig:modelDesc}.
...
\textsc{HS-Prior-LDA}, \textit{history states}, and \textit{recent states} forms up our proposed event detection method \textsc{LTDetector}.
...
Event phase \(\mathcal{C}_{c,t,i}\) is the \(i\)-th combination of words which occurred in the set of events' candidate words \(\mathcal{B}_{c,t}\), and represents the \(i\)-th event happened in time \(t\) under the category \(c\).
\end{rmk}
...
\scalebox{0.95}{
    ...
    \textsc{LTDetector} & 1.000 & 0.894 & 0.950 & 0.042 \\ \hline
    \end{tabular}
}
