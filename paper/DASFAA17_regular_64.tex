...
\author{Weijing Huang\inst{1} \and Tengjiao Wang\inst{1} \and Wei Chen\inst{1} \thanks{Corresponding author.} \and Yazhou Wang\inst{1} }
\institute{\(^{1}\)Key Laboratory of High Confidence Software Technologies (Ministry of Education), EECS, Peking University, Beijing, China \\ 
\path{huangwaleking@gmail.com,{tjwang,pekingchenwei}@pku.edu.cn}\\ 
...
\keywords{event detection, microblog stream, transfer learning, knowledge base.}
\end{abstract}
...
%Transfer learning uses the knowledge in the source domain to enhance the performance of learning tasks on the target domain. \cite{dai2007transferring}
...
In our proposed \textsc{TransDetector} method, by using category-level topic in knowledge base as the prior knowledge, we transfer abundant knowledge from knowledge base into microblog stream, which enriches the semantics of microblogs and further enhances the accuracy of event detection for microblogs. 
